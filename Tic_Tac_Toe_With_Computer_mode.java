...
public class Tic_Tac_Toe_With_Computer_mode {
    static int count1 = 0;
    static int count2 = 0;
    ...
    static boolean cmode(){
        Scanner input = new Scanner(System.in);
        System.out.println("Do you want to play in computer mode?");
        System.out.println("Enter your choice Y for 'yes' and N for 'no' -->");
        String c = input.next();
        if(c.equals("Y")){
            return true;
        }
    ...
    static void ComInput(String[][] board,int [][] nums,String p,String ch){
        Scanner input = new Scanner(System.in);
        Random rand = new Random();
        Display(board);
        System.out.println(p + "'s turn -- >(" + ch + ")");
        int pos = 1 + rand.nextInt(9);
        ComIndexFinder(board,nums,p,pos,0,2,ch);
    }

    static void ComIndexFinder(String[][] board,int[][] nums,String p,int x,int i,int j,String ch){
        while(i<3 && j>=0){
            if(nums[i][j]>x){
                j--;
            }
        ...
        if(board[i][j].equals("X") || board[i][j].equals("O")){
            ComInput(board, nums, p, ch);
        }
        else{
            board[i][j] = ch;
            System.out.println(p + "'s input is : " + x);
        }
    }
